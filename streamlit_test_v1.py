...
def main():
    ...
    if video_file is not None:
        # Read video file
        video = cv2.VideoCapture(video_file.name, cv2.CAP_FFMPEG)
        # video = ffmpegcv.VideoCapture(video_file.name)
        # st.write(video.isOpened())
        # video.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        # video.set(cv2.CAP_PROP_FRAME_HEIGHT, 352)
